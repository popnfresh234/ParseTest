...
// Use the QueryFactory to construct a PQA that will show all restaurants
        ...
        super(context, new ParseQueryAdapter.QueryFactory<Restaurant>() {
            public ParseQuery create() {
                ...
                if (queryCode == ListActivityFragment.FAVORITES) {
                    ParseUser user = ParseUser.getCurrentUser();
                    ParseRelation<Restaurant> relation = user.getRelation(ParseConstants.RELATION_FAVORITE);
                    query = relation.getQuery();
                }

                if (queryCode == ListActivityFragment.MY_RESTAURANTS) {
                    query.whereEqualTo(ParseConstants.KEY_AUTHOR, ParseUser.getCurrentUser());
                    query.addAscendingOrder(ParseConstants.KEY_RESTAURANT_LOWERCASE_TITLE);
                }

                if (queryCode == ListActivityFragment.RECENT_RESTAURANTS_ONE_DAY) {
                    Calendar calendar = Calendar.getInstance();
                    calendar.add(Calendar.DAY_OF_YEAR, -1);
                    Date newDate = calendar.getTime();

                    query.whereGreaterThan(ParseConstants.KEY_CREATED_AT, newDate);
                    query.addAscendingOrder(ParseConstants.KEY_RESTAURANT_LOWERCASE_TITLE);
                }

                if (queryCode == ListActivityFragment.RECENT_RESTAURANTS_ONE_WEEK) {
                    Calendar calendar = Calendar.getInstance();
                ...
                if (queryCode == ListActivityFragment.RECENT_UPDATE) {
                    Calendar calendar = Calendar.getInstance();
                    calendar.add(Calendar.DAY_OF_YEAR, -1);
                    Date newDate = calendar.getTime();
                    query.whereGreaterThan(ParseConstants.KEY_UPDATED_AT, newDate);
                    query.addAscendingOrder(ParseConstants.KEY_RESTAURANT_LOWERCASE_TITLE);
                }
                return query;
